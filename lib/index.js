 ...
 */
 ...
 var jsonandxml = (() => {
    ...
    this.getData = async (data, web) => {
      if(typeof data == 'object') return this.convertData(data, web)
      if(typeof data == 'string') return data;
      throw 'invalid filedata';
    }

    this.convertData = async (filedata, web) => 
      web ? await this.webFileReader(filedata) : await this.readFile(filedata, true)
    
 
    this.dataValidation = (data, type) => {
      if(!data || data.length==0) throw 'No DATA send';
      if(!type || type.length==0) throw 'No TYPE send';
    ...
    this.isitJson = data => {
      if(data.indexOf('<') == 0) {
        const validatedData = this.dataValidation(data, 'xml');
        return xmltojson.convert(validatedData);
      } else {
        const validatedData = this.dataValidation(data, 'json');
        return jsontoxml.convert(validatedData);
      }
    }
