...
class transform {
  ...
  static convert(glyph) {
    ...
    glyph.forEach(point => {
      const parsed = transform.assign(point, GX, GY);
  ...
  static assign(point, gx, gy) {
    ...
    switch(point[0]) {
      ...
      case 'h':
      case 'H': return transform.h(point, gx, isAbs);
      case 'v':
      case 'V': return transform.v(point, gy, isAbs);
      case 'l':
      ...
      case 's':
      case 'S': return transform.s(point, gx, gy, isAbs);
      case 'q':
      case 'Q': return transform.q(point, gx, gy, isAbs);
      case 't':
      case 'T': return transform.t(point, gx, gy, isAbs);
      case 'z':
  ...
  static l(point, gx, gy, isAbs) {
    return ({
      name: 'L',
      x: (isAbs ? 0 : gx) + point[1],
      y: (isAbs ? 0 : gy) + point[2],
    });
  }
...
generateTTF(
  [{"tag":"glyph","attributes":{"d":"M 10 80 q 42.5 -70, 85 0 t 85 0"},"content":"\n","selfClosing":true}]
)
