...
class extractor {
  ...
  static styleClasses(content) {
    ...
    let classNames = []
    let count = 0
    let test = exploit(this.content(content).replaceAll(/[^n{a-zA-Z0-9}.:;,]/g, ''), /}./g).map(item => item.replaceAll(/[{}]/g, ' ').trim()).map(item => exploit(item, /[ ]/g)).map(item => item.map(item2 => exploit(item2.replaceAll(';', ' '), /[ ]/g)))
    console.log(test);
    exploit(this.content(content).replaceAll(/[^n{a-zA-Z0-9}.:;,]/g, '').replaceAll(/[{}]/g, ' '),/[ ]/g)
    .forEach((item, i) => {
      if (i % 2 == 0) {
        latestClass = item
        classNames.push(latestClass)
        styles[latestClass] = []
      } else 
        styles[latestClass] = exploit(item.replaceAll(/[;]/g, ' '), / /g).map(item => exploit(item.replaceAll(/[;:]/g, ' '), / /g))
    })
    return styles
  }
