...
class extractor {
  ...
  static styleClasses(content) {
    return exploit(this.content(content), /}./g).map(cssClass => {
      let css = exploit(cssClass, /{/g);
      let name = css[0].replace(/[{}]/g, '')
      let style = exploit(css[1].replace(/[{}]/g, ''), /[;]/g)
        .map(style => style.replace(/[;]/g, ''))
        .map(style => exploit(style, /[:]/g)
          .map(cssProp => cssProp.replace(/[:]/g, ''))
        ).map(style => style
          .map(value => value.match(/[n{a-zA-Z}]/g) ? value : parseFloat(value))
        );
      return ([name, style])
    })
  }
