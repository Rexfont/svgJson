...
function assign(glyph, output) {
  ...
  let tmpglyph = glyph;
  let lastMCommand = {}
  tmpglyph.forEach((command, i) => {
    // convert
    if (command[0].toLowerCase() == 'z') {
      GX = lastMCommand.localx
      GY = lastMCommand.localy
    }
    const parsed = process(command, output, GX, GY);
    if (command[0].toLowerCase() == 'm') lastMCommand = parsed
