...
export default function parsePath(data, {unifySvg}) {
  ...
  .then(data => data.map(tagData => {
    if (isToBeConverted(tagData.tag)) {
      const attrs = tagData.attributes;
      tagData.attributes = { ...attrs, d: assign(tagData.tag, attrs), class: attrs && attrs.class || null, id: attrs && attrs.id || null }
      tagData.tag = 'path'
    }
    return tagData;
  }));
