...
function parseSvgfont(data, unicodePrefix, fontname) {
  ...
  .then(async data => {
    ...
    // const horizAdvX = largestHorizAdvX(glyphs)
...
function stackthedata({glyphs, styles, fontname='rexfont', unicodePrefix='RX', dimentions}) {
  ...
  content += `<font-face 
    ...
    descent="${0}"
    />`;
  content += `<missing-glyph horiz-adv-x="${dimentions.width}" />\n`;
  let rxcode = 0;
  let unicodeBase = ''
  glyphs.forEach((glyph, i) => {
    ...
    `<glyph
      ${glyph.attributes && glyph.attributes.class ? 
        tools.exploit(glyph.attributes.class, /[ ]/g).map(sclass => styles[sclass.indexOf('.')==0 ? sclass : `.${sclass}`].map(item => item.join('="')).join('" ')+'"').join(' ')
        : ""
      }
