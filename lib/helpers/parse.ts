...
import {exploit, removecommas} from './tools';

export function parseContourTag(tag) {
  const contours = [];
...
export function parseContourSvg(svg) {
  return svg.map(tag => parseContourTag(tag))
}

export async function parseContourAsync(path) {
  return parseContourPath(path)
}

export function exploitFullD(data) {
  if(!data) return null;
  return exploit(data, /[n{a-zA-Z}]/g).map(exploited => specs(removecommas(exploited))).flat();
}
 ...
 */
export function getMaxMin(contours) {
...
export function attachStrContour(font) {
...
export function attachStrContourDirectly(contours) {
  let string = '';
  contours.forEach((contour) => {
...
export function parseContourPath(path) {
  const contours = [];
...
export function useContourStr(font) {
...
export async function attachStrContourAsync(font) {
  return attachStrContour(font);
}

export function exploitSingleD(data) {
  const symbol = data[0];
...
export async function pathParser(font) {
...
export function glyphMatrix(contours, a, b, c, d, e, f) {
  let prevX=0, prevY=0;
...
export function roundup(contoursArr) {
...
export function getClassesEncode(classes) {
  const list = {}
  let count = 0
...
export function encodeClasses(svgsJsons) {
...
export function extractPathes(svgJsons) {
  const pathes = []
...
export function extractGlyphSets(json) {
  const glyphSets = {};
  json.forEach(tag => {
...
export function extractStyles(svgJsons) {
  return xjs.mapIf(svgJsons, 'tag.style', true)
}

export function extractGlyphs(json) {
  const glyphs = [];
  json.forEach(tag => {
